...
std::vector<RippleAddress> SerializedTransaction::getMentionedAccounts () const
{
    ...
    for (const SerializedType & it : peekData ())
    {
        ...
        if (sa != nullptr)
        {
            ...
            for (const RippleAddress & it : accounts)
            {
                if (it == na)
                {
                    found = true;
...
{
    return STObject::getSigningHash (stellar_mobile::Config::DataContainer.SIGN_TRANSACTION);
}
