...
Json::Value STPath::getJson (int) const
{
    ...
    for (std::vector<STPathElement>::const_iterator::value_type it : mPath)
    {
...
Json::Value STPathSet::getJson (int options) const
{
    ...
    for (std::vector<STPath>::const_iterator::value_type it : value)
...
void STPathSet::add (Serializer& s) const
{
    ...
    for (const STPath & spPath : value)
    {
        ...
        for (const STPathElement & speElement : spPath)
        {
