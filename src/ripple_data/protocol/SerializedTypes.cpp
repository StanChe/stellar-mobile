...
STPath::STPath(Json::Value json) 
{
	Json::FastWriter writer;
	for (unsigned int i = 0; i < json["paths_computed"].size(); i++)
	{
		uint160 account;
		uint160 uDstCurrency;
		uint160 uDstIssuer;
		Json::Value path = json["paths_computed"][i];
		for (unsigned int j = 0; j < path[i].size(); j++) 
		{
			int iType = path[j][jss::type].asInt();
			//todo wft!??
			if (iType == 1)
				account = RippleAddress::createAccountID(path[j][jss::account].asString()).getAccountID();
			if (iType == 48)
			{
				STAmount::currencyFromString(uDstCurrency, path[j][jss::currency].asString());
				STAmount::issuerFromString(uDstCurrency, path[j][jss::issuer].asString());
			}
		}
		STPathElement elem = STPathElement(account, uDstCurrency, uDstIssuer);
		addElement(elem);
	}
}

STPathSet::STPathSet(Json::Value json) 
{
	for (unsigned int i = 0; i < json["alternatives"].size(); i++)
	{
		addPath(STPath(json["alternatives"][i]));
	}
}
#if 0
